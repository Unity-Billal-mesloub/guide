...
For more specific changes, please read the [History.md](https://github.com/Unity-Billal-mesloub/component/blob/main/History.md).
...
For a while, we used a [wiki](https://github.com/Unity-Billal-mesloub/component/wiki/Components) to keep track of components,
...
Not all the maintainers of the [component](https://github.com/Unity-Billal-mesloub) organization use Component - many use `npm` and `browserify`.
...
Component now has a `component build --watch` option, allowing incremental builds. It uses [sane](http://github.com/Unity-Billal-mesloub/sane) under the hood, which seems to be one of the best JS watcher libraries currently published.
...
see [example](https://github.com/Unity-Billal-mesloub/c8-experiments/blob/e7a/lib/my-component/component.json#L17-L34). If you use mocha as test runner you can setup your `test.html` like this:
...
Feel free to add more [remotes](https://github.com/Unity-Billal-mesloub/remotes.js)!
...
- `component create` - has moved to https://github.com/Unity-Billal-mesloub/create.js. It may be re-added at a later date after improvements are made.
...
The `component.json` [specifications](https://github.com/Unity-Billal-mesloub/spec/blob/main/component.json/specifications.md) have changed over time.
...
- `.remotes` now handles an array of strings which refer to an remote implementation: `'github'` and `'bitbucket'`, see [spec](https://github.com/Unity-Billal-mesloub/spec/blob/main/component.json/specifications.md#remotes)
...
- The canonical require path has changed. If you want to require locals which are not the first (boot) component, you need the canonical path. If you have main component like [this](https://github.com/Unity-Billal-mesloub/boilerplate-express/blob/main/component.json), then the canonical path is `./client/boot`. Non-[main files](https://github.com/Unity-Billal-mesloub/spec/blob/main/component.json/specifications.md#main) of a local need to be required with the file name __and__ the file extension, for instance: `./client/boot/helper.js`.
A use case for canonical paths is to require test files for the development environment (`component build --dev`), [see example](https://github.com/Unity-Billal-mesloub/c8-experiments/blob/e7a/test/index.html#L18).
